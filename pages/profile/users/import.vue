<template>
  <div class="space-y-6">
    <!-- Back Button -->
    <div>
      <UButton to="/profile/users" color="gray" variant="ghost" icon="i-heroicons-arrow-left">
...
function autoMapByName() {
  // Try to match device names to existing user discord names
  for (const device of appleDevices.value) {
    const deviceNameLower = device.name.toLowerCase()
    for (const user of existingUsers.value) {
      if (deviceNameLower.includes(user.discordName.toLowerCase())) {
        mappings[device.udid].mode = 'existing'
        mappings[device.udid].discordName = user.discordName
        break
      }
    }
  }
